...
type CommandViewForApp struct {
	Views
}

func (this *CommandViewForApp) Auth(ctx iris.Context) int {
	this.Views.Auth(ctx)
	var perms = PMS{
		"PUT":    MA{"CUSTOMER": A{1}},
		"GET":    MA{"CUSTOMER": A{1}},
...
//deviceId int 设备号
func (this *CommandViewForApp) Post(ctx iris.Context) (statuCode int, data M) {
	statuCode = 400
	data = make(M)
	...
	if err != nil {
		data["code"] = 0
		data["msg"] = "不存在该设备,请联系工作人员"
		data["data"] = nil
		return
	}
	if deviceData.DeviceUser.UserId != mem.UserId {
		data["code"] = 0
		data["msg"] = "发送指令受限(该设备不属于此操作人)"
		data["data"] = nil
		return
	}
	...
	if cmd_type == 1 { //设备开启追踪
		if mod.Model_type == 0 {
			...
			if err != nil {
				data["code"] = 0
				data["msg"] = "追踪开启失败,请联系工作人员"
				data["data"] = nil
				return
			}
			if track_min < 1 || track_min > max {
				data["code"] = 0
				data["msg"] = "追踪时间超过上限,请重新设置"
				data["data"] = nil
				return
			}
	...
	statuCode = 200
	data["data"] = map[string]interface{}{"result": trackInterval}
	data["code"] = 1
	data["msg"] = "OK"
	return
}
